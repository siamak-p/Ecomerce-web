...
# def converting():
#     _, year, month, day = jalali_date_converter_latin(timezone.now())
#     dt = year, month, day
#     return dt
#
#
...
def checkout(request):
    ...
    # agar shipping address baraye karbare jari vojood nadashte bashad else ejra mishavad
        ...
        if request.method == 'POST':
            ...
            if shipping_form.is_valid():
                print('-----------------------------------------------')
                print('valid: ', shipping_form.is_valid())
                customer = request.user
                order = order
    ...
    elif request.method == 'POST' and addressID is None and sending_time is None and payment_method is None:
    ...
    if request.method == 'POST' and payment_method == 'offline-method':
        # order.delivery_time = datetime2jalali(timezone.now())
        ...
        # redirect('/cart/final/')
        fcontext = {'code': random_code}
        print('--------------------------------------------------------')
        print('its ok')
        print('--------------------------------------------------------')
        return render(request, 'final.html', fcontext)
